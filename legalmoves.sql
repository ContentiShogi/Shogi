...
DECLARE @moves TABLE ([mover] [bit] NULL,[koma] [int] NOT NULL,[position] [int] NULL,[moves] [int] NULL,[promotion] [smallint] NULL,[moves_id] [int] NOT NULL
	,[movesuji] [int] NULL,[movedan] [int] NULL,[movekakupara] [int] NULL,[movekakuorth] [int] NULL,bitmask int null,direction tinyint,distance tinyint null)
...
with threatranges as
	...
	, rangelimits as
	( select threatranges.mover,threatranges.koma,threatranges.position,direction,min(distance)mindist
		from threatranges
		join @kyokumen kyokumen on kyokumen.position=threatranges.moves
		group by threatranges.mover,threatranges.koma,threatranges.position,threatranges.direction
	)
	delete moves1
	from @moves moves1
	--join threatranges on moves1.moves_id=moves1.moves_id
	join rangelimits on rangelimits.mover = moves1.mover and rangelimits.koma=moves1.koma
		and rangelimits.position = moves1.position and rangelimits.direction = moves1.direction
	where moves1.distance > rangelimits.mindist;
